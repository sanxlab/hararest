...
export class FacebookService {
    ...
    private async getVideoInfoFromYtDlp(url: string): Promise<FacebookVideoInfo> {
        try {
            const run = await execFilePromise(config.youtube.binPath,
                ['--ignore-config', '--no-playlist', '--skip-download', '--socket-timeout', '15',
                    '--retries', '1', '--dump-single-json', '--', url],
            ...
            const formats = Array.isArray(data?.formats) ? data.formats.filter((format) =>
                format && typeof format.url === 'string' && /^https?:\/\//.test(format.url) &&
                format.ext === 'mp4' && format.acodec !== 'none' && format.vcodec !== 'none') : [];
            const unique = formats.filter((format, index) => formats.findIndex((item) => item.url === format.url) === index);
            if (!unique.length) throw new AppError('No downloadable Facebook video found.', 404);
            const videos = await Promise.all(unique.map(async (format) => {
                const size = Number.isSafeInteger(format.filesize) && (format.filesize || 0) > 0
                    ? format.filesize as number : await getMediaSize(format.url as string);
                return {
                    quality: format.format_id?.toLowerCase().includes('hd') || (format.height || 0) >= 720 ? 'hd' : 'sd',
                    url: format.url as string, size, fSize: bytesToSize(size)
                };
            }));
