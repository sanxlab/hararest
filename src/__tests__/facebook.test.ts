  ...
  describe('GET /api/facebook', () => {
    it('should return video info from SnapSave extractor', async () => {
      ...
      const response = await supertest(app).get(
        '/api/facebook?url=https://facebook.com/watch?v=123',
      );
...
it('returns an upstream error when SnapSave fails without invoking yt-dlp', async () => {
  mockedExecFile.mockImplementation((...args: unknown[]) => {
    const callback = args[args.length - 1] as ExecFileCallback;
    callback(
      Object.assign(new Error('SnapSave failed'), { stderr: '{"message":"Robot Check"}' }),
      '',
      '{"message":"Robot Check"}',
    );
    return {} as never;
  ...
  const response = await supertest(app)
    .get('/api/facebook')
    .query({ url: 'https://facebook.com/watch?v=123' });
  expect(response.status).toBe(502);
  expect(response.body.message).toContain('SnapSave');
